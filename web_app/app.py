...
def create_app():
    ...
    # Загрузка профилей
    def load_profiles():
        # Проверяем файл в текущей директории, а затем в родительской
        data_file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), DATA_FILE)
        if os.path.exists(data_file_path):
            with open(data_file_path, 'r', encoding='utf-8') as f:
                return json.load(f)
        else:
            # Также проверяем файл в текущей директории
            local_data_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), DATA_FILE)
